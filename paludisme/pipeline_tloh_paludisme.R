...
tloh_2025_brute <- read_excel(here("paludisme", "SYNTHESE TLOH 2025_BLMG.xlsx"), 
                        col_names = FALSE)
tloh_2026_brute <- read_excel(here("paludisme", "SYNTHESE_TLOH_2026_DS_BLMG.xlsx"),
                              sheet = "Base TLOH 2026",col_names = FALSE) |> 
  slice(-3) # ou tloh_2026[-3, ] pour suprimer la ligne 3 qui est vide 
...
#     Propager le nom de maladie sur les cellules fusionnées vides  -> forward fill (#20)
#2025
...
## Bloc 2.2 table 2026
ligne_1a <- unlist(tloh_2026_brute[1, ])
ligne_2b <- unlist(tloh_2026_brute[2, ])

for(i in 2:length(ligne_1a)){
  if (is.na(ligne_1a[i])){
    ligne_1a[i] <- ligne_1a[i-1]
  }
}
names_2 <- paste(ligne_1a, ligne_2b, sep = "_")
...
# Nettoyer la colonne semaine "S1", 
  mutate(
    ...
    across(where(is.character), as.numeric), #transformer les colonnes en numeric
    ) |> 
# Exclure les semaines non effectives (23 lignes attendues)
...
#Remplacer les NA par zero (NA = pas de notification = 0 cas)
if(exist_NA !=0){
...
#     Agrégation par année  -> group_by + summarise (#17), across (#43)
#     [NOUVEAU] létalité = ratio, attention division par zéro (cas = 0 -> NA ou Inf à gérer)
#     Sortie : petit tibble de synthèse par année (et par semaine pour les courbes)
# 
#   [NOTE] Le bloc "agrégation par formation sanitaire / 48 FS" de ta trame tierce
#          N'EST PAS POSSIBLE ICI : ces synthèses sont déjà agrégées au niveau district,
#           pas désagrégées par FS. Reporté à quand tu joindras les données par FS.
#           
#           
#           PHASE 3 — VISUALISATION (ggplot2)
#           
#           BLOC 6 — Courbe d'évolution hebdomadaire 2025 vs 2026
#     Cas de paludisme par semaine, une courbe par année  -> ggplot + geom_line (#28), group/color = annee (#29)
